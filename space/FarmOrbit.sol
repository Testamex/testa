...
import "./Orbit.sol";
import "./interfaces/ITesta.sol";

contract FarmOrbit is Ownable, ReentrancyGuard, Orbit {
    /// @notice Libraries
    ...
    event Destruct(uint256 indexed id, uint256 wad);
    ...
    address public refuelHoldingToken;
    uint256 public refuelHoldingTokenAmount;
    address public destructHoldingToken;
    uint256 public destructHoldingTokenAmount;
    ...
    constructor(
        ...
        address _refuelHoldingToken,
        uint256 _refuelHoldingTokenAmount,
        address _destructHoldingToken,
        uint256 _destructHoldingTokenAmount
    ) public {
        operator = _operator;
        weth = _router.WETH();
        farm = _farm;
        ...
        refuelHoldingToken = _refuelHoldingToken;
        refuelHoldingTokenAmount = _refuelHoldingTokenAmount;
        destructHoldingToken = _destructHoldingToken;
        destructHoldingTokenAmount = _destructHoldingTokenAmount;
        // Get lpToken and fToken from testa pool
        pid = _pid;
        ...
        lpToken.approve(address(_farm), uint256(-1)); // 100% trust in the staking pool
        lpToken.approve(address(router), uint256(-1)); // 100% trust in the router
        testa.safeApprove(address(router), uint256(-1)); // 100% trust in the router
    }
    ...
    modifier refuelModifier() {
        require(IERC20(refuelHoldingToken).balanceOf(msg.sender) >= refuelHoldingTokenAmount, "insufficient token hoding");
        _;
    }

    function setRefuelHoldingToken (address _refuelHoldingToken) public onlyOwner {
        refuelHoldingToken = _refuelHoldingToken;
    }

    function setRefuelHoldingTokenAmount(uint256 _refuelHoldingTokenAmount) public onlyOwner {
        refuelHoldingTokenAmount = _refuelHoldingTokenAmount;
    }

    function setDestructHoldingToken (address _destructHoldingToken) public onlyOwner {
        destructHoldingToken = _destructHoldingToken;
    }

    function setDestructHoldingTokenAmount(uint256 _destructHoldingTokenAmount) public onlyOwner {
        destructHoldingTokenAmount = _destructHoldingTokenAmount;
    }
    ...
    function shareToBalance(uint256 share) public view returns (uint256) {
        ...
        (uint256 totalBalance) = farm.userInfo(pid, address(this));
    ...
    function refuel() public onlyEOA nonReentrant refuelModifier {
        ...
        farm.deposit(pid, lpToken.balanceOf(address(this)));
